...
<x-layouts.public :meta="$meta ?? []">
    <section class="py-24 md:py-32 px-4 relative overflow-hidden pt-36">
        <div class="max-w-5xl mx-auto relative z-10 text-center">
            <span class="text-accent font-black tracking-[0.3em] text-xs uppercase">{{ $isEs ? 'Programa de Recompensas' : 'Rewards Program' }}</span>
            <h1 class="text-white font-heading text-5xl md:text-8xl tracking-tighter uppercase mt-6 mb-6 leading-none">{{ $isEs ? 'Gana Puntos.' : 'Earn Points.' }}<br><span class="text-accent italic">{{ $isEs ? 'Obtén Recompensas.' : 'Get Rewards.' }}</span></h1>
    ...
    <section class="px-4 pb-8">
        <div class="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="bg-white/5 border border-white/10 rounded-[30px] p-8 md:p-12 text-center">
                ...
                <h3 class="text-white font-heading text-2xl font-black uppercase mb-4">{{ $isEs ? 'Regístrate' : 'Sign Up' }}</h3>
                <p class="text-white/40 text-sm">{{ $isEs ? 'Únete gratis al programa A1 Black Member. No hay costo de membresía — solo beneficios.' : 'Join A1 Black Member for free. No membership cost — just benefits.' }}</p>
            </div>
            <div class="bg-white/5 border border-white/10 rounded-[30px] p-8 md:p-12 text-center">
                <div class="w-16 h-16 rounded-full bg-accent flex items-center justify-center mx-auto mb-6">
                    <span class="text-black font-heading text-3xl font-black">2</span>
                </div>
                <h3 class="text-white font-heading text-2xl font-black uppercase mb-4">{{ $isEs ? 'Gana Puntos' : 'Earn Points' }}</h3>
                <p class="text-white/40 text-sm">{{ $isEs ? 'Gana 1 punto por cada $1 que gastes en sesiones de entrenamiento, terapia física y más.' : 'Earn 1 point for every $1 you spend on training sessions, physical therapy, and more.' }}</p>
            </div>
            <div class="bg-white/5 border border-white/10 rounded-[30px] p-8 md:p-12 text-center">
                <div class="w-16 h-16 rounded-full bg-accent flex items-center justify-center mx-auto mb-6">
                    <span class="text-black font-heading text-3xl font-black">3</span>
                </div>
                <h3 class="text-white font-heading text-2xl font-black uppercase mb-4">{{ $isEs ? 'Canjea' : 'Redeem' }}</h3>
                <p class="text-white/40 text-sm">{{ $isEs ? '5 puntos = 10% de descuento en tu próxima compra. Acumula y canjea cuando quieras.' : '5 points = 10% off your next purchase. Stack and redeem whenever you want.' }}</p>
            </div>
        </div>
    </section>
    ...
    <section class="px-4 pb-24">
        <div class="max-w-3xl mx-auto bg-accent rounded-[30px] p-8 md:p-12 text-center">
            <h2 class="text-black font-heading text-3xl font-black uppercase mb-4">{{ $isEs ? 'Términos' : 'Terms' }}</h2>
            <ul class="text-black/60 text-sm space-y-2 text-left max-w-md mx-auto">
                <li class="flex items-start gap-2"><span class="text-black font-black">•</span> {{ $isEs ? 'Válido para todos los servicios de pago de A1 Training Group.' : 'Valid on all paid A1 Training Group services.' }}</li>
                <li class="flex items-start gap-2"><span class="text-black font-black">•</span> {{ $isEs ? 'Los puntos no caducan mientras la membresía esté activa.' : 'Points never expire while membership is active.' }}</li>
                <li class="flex items-start gap-2"><span class="text-black font-black">•</span> {{ $isEs ? 'No se puede combinar con otras promociones.' : 'Cannot be combined with other promotions.' }}</li>
            </ul>
        </div>
    </section>
