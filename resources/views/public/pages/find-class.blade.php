    ...
    <section class="py-24 md:py-32 px-4 relative overflow-hidden pt-36">
        <div class="max-w-4xl mx-auto relative z-10">
            <h1 class="text-white font-heading text-4xl sm:text-5xl md:text-7xl tracking-tighter uppercase mb-6 leading-[1.1]">
                {{ $isEs ? 'Encuentra tu Clase' : 'Find Your Class' }}
            </h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
                <a href="{{ $p }}/services/personal-training" class="bg-white/5 border border-white/10 rounded-[30px] p-8 hover:border-accent/40 transition-all">
                    <h3 class="text-white font-heading text-2xl font-black uppercase mb-2">{{ $isEs ? 'Entrenamiento Personal' : 'Personal Training' }}</h3>
                    <p class="text-white/40 text-sm">{{ $isEs ? '1-a-1 en estudio, en casa o en línea.' : '1-on-1 at studio, home, or online.' }}</p>
                </a>
                <a href="{{ $p }}/services/boxing" class="bg-white/5 border border-white/10 rounded-[30px] p-8 hover:border-accent/40 transition-all">
                    <h3 class="text-white font-heading text-2xl font-black uppercase mb-2">{{ $isEs ? 'Boxeo' : 'Boxing' }}</h3>
                    <p class="text-white/40 text-sm">{{ $isEs ? 'Cardio de alta intensidad y técnica.' : 'High-intensity cardio and technique.' }}</p>
                </a>
                <a href="{{ $p }}/services/physical-therapy" class="bg-white/5 border border-white/10 rounded-[30px] p-8 hover:border-accent/40 transition-all">
                    <h3 class="text-white font-heading text-2xl font-black uppercase mb-2">{{ $isEs ? 'Terapia Física' : 'Physical Therapy' }}</h3>
                    <p class="text-white/40 text-sm">{{ $isEs ? 'Rehabilitación experta y prevención.' : 'Expert rehab and injury prevention.' }}</p>
                </a>
                <a href="{{ $p }}/services/consultations" class="bg-accent text-black rounded-[30px] p-8 hover:bg-white transition-all">
                    <h3 class="font-heading text-2xl font-black uppercase mb-2">{{ $isEs ? 'Consulta Gratuita' : 'Free Consultation' }}</h3>
                    <p class="text-black/60 text-sm">{{ $isEs ? 'Comienza con una evaluación sin compromiso.' : 'Start with a no-obligation assessment.' }}</p>
                </a>
            </div>
        </div>
    </section>
